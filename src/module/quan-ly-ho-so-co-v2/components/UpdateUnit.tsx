...
export function UpdateUnit({ isDone }: { isDone?: boolean }): JSX.Element {
      ...
      onSuccess: async (data) => {
        if (data.results) {
          ...
          const newSanPhams = produce(sanPhams, (draft) => {
            if (ma_hh === draft[spIndex].ma_hh && stk === draft[spIndex].stk) {
              draft[spIndex]["nguyen_lieu"] = mappingNVL(nguyenLieu);

              let countNoMapping = 0;
              draft[spIndex]["nguyen_lieu"]?.forEach((i: any) => {
                if (
  ...
  const checkIsFullUnit = () => {
    let result = true;
    normVatImport?.forEach((item: any) => {
  ...
  const checkIsNeedTransUnit = () => {
    let result = false;
    normVatImport?.forEach((item: any) => {
      if (item.unit_import_vat !== item.unit_norm) {
        result = true;
        return;
      }
    });
  ...
  return (
    <>
      <Tooltip
        title={
          checkIsNeedTransUnit()
            ? checkIsFullUnit()
              ? "Chuyển đổi đơn vị"
              : "Bạn cần chuyển đổi đơn vị"
            : "Không cần chuyển đổi"
        }
      >
        <Button
          disabled={isDone || !checkIsNeedTransUnit()}
          onClick={handleOpen}
          style={{ zIndex: 2 }}
        >
          <SyncOutlined
            spin={!checkIsFullUnit() && checkIsNeedTransUnit()}
            style={{ color: checkIsFullUnit() ? "#1890ff" : "#f4aa3a" }}
          />
          <Typography.Text
            type={
              !checkIsFullUnit() && checkIsNeedTransUnit()
                ? "danger"
                : undefined
            }
            strong={!checkIsFullUnit() && checkIsNeedTransUnit()}
          >
            {!checkIsFullUnit() && checkIsNeedTransUnit()
              ? "Bạn cần chuyển đổi đơn vị"
              : "Chuyển đổi đơn vị"}
