...
import { CTSHTable } from "../tables/ctsh-table";
import { calculateTotalOfWeightCTC, isDoneCoDocument } from "../utils";

const { Text } = Typography;

export default function Ctsh({
  keyCriterial = "ctsh",
}: {
  keyCriterial?: string;
}): JSX.Element {
  ...
  const values = {
    ...
    value_none_xx:
      ...
      calculateTotalOfWeightCTC(sanPham, 6, chooseFormCo),
  ...
  // const resultCTSH = handleTotalChiPhiNguyenLieu(
  //   sanPham?.nguyen_lieu,
  //   sanPham?.otherFees,
  // );
  ...
  const handleChange = (e: any) => {
    ...
    const newSanPhams = produce(sanPhams, (draftState) => {
      ...
      sanPham.tcs[keyCriterial].end = handleCalcEndCtsh(
        { ...values, [name]: value },
        sanPham?.fob_value,
      );
    });
  ...
  useEffect(() => {
    ...
    const check =
      handleCalcEndCtsh(sanPham?.tcs?.[keyCriterial], sanPham?.fob_value) ===
        0 ||
      ...
      lackedMaterial === 0 ? (
        ...
        <div className={styles.ctc_top}>
                        ...
                        rules={[
                          {
                            required: true,
                            message:
                              "Tổng trọng lượng của thành phẩm không được bỏ trống",
                          },
                        ]}
                      >
                        <Input
          ...
          <Row className={styles.ctc_top}>
            ...
            <Col span={15} push={1}>
              {handleCalcEndCtsh(values, sanPham?.fob_value) === 0 ? (
                <Text type="success" strong>
                  {t("for_all.achieved").toUpperCase()} TIÊU CHÍ{" "}
...
export function handleCalcEndCtsh(values: any, result: any) {
