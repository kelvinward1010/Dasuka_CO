...
import { DataTb2RvcCTState } from "@/store/datanvl/atom";
import { total2RvcState } from "@/store/total/atom";
...
export default function ChiPhiPhanCongttTable(props: dataTable): JSX.Element {
  const [data, setData] = useState<any[]>(props?.dt || []);
  const [, setTotal] = useRecoilState(total2RvcState);
  const [, setDataX] = useRecoilState<any[]>(DataTb2RvcCTState);
  ...
  return (
    <>
      <Text strong>
        {`2. Chi phí phân công trực tiếp (${decimalUSD.format(
          Number(totalmain) ?? 0,
        )})`}
      </Text>
      <>
