...
import { useUpdateExportExpected } from "../api/updateExportDeclaration";
...
export function UpdateExportDeclaration({
  ...
  const listDetails = Form.useWatch("export_declaration_detail", form);
  ...
  const { data, isLoading } = useExportDeclaration({
    ...
    config: {
      ...
      onSuccess: (data) => {
        ...
        setDateExport(dayjs(data.date_of_declaration || now()));
        form.setFieldsValue(data);
      },
    },
  });

  const updateExportDeclaration = useUpdateExportExpected({
    config: {
      onSuccess: () => {
    ...
    form
      ...
      .then((values) => {
        updateExportDeclaration.mutate({
          ...
          export_declaration_detail: null,
          export_declaration_id: exportDeclaration.export_declaration_id,
          date_of_declaration: dayjs(dateExport).format("YYYY-MM-DD"),
          lu_user_id: userRecoil.user_id,
          list_json_export_declaration_detail: getUpdatedArray(
            data?.export_declaration_detail || [],
            listDetails.map((item: any) => ({
              ...item,
              origin_country: "Việt Nam",
              export_declaration_id: exportDeclaration.export_declaration_id,
            })),
            "export_declaration_detail_id",
          ).map((i) => ({
            ...i,
            created_by_user_id:
              i.status === "1" ? userRecoil.user_id : i.created_by_user_id,
          })),
        });
      })
      .catch(() => {
      ...
      >
        ...
        >
          <Typography.Title level={4} className={styles.modal_header_title}>
            {t("export_declaration.detail.title_expected")}
          </Typography.Title>
          <Col>
            <CloseCircleOutlined
...
export function UpdateExportDeclarationTable({
  ...
  const handleSelectCode = async (key: number, option: any) => {
    ...
    const list = form.getFieldValue("export_declaration_detail");

    list[key] = { ...list[key], ...values };

    form.setFieldValue("export_declaration_detail", list);
  };
