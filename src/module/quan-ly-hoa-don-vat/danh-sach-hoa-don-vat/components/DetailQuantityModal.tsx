...
import { useQuantityVat } from "../api/getQuantityVat";
import styles from "../style.module.scss";
import { IQuantityVat, VatInvoice, VatInvoiceDetail } from "../types";

interface Props {
  vatInvoice: VatInvoice | undefined;
  vatInvoiceDetail: VatInvoiceDetail;
  sort_order: number;
  value: number | string;
}
...
export function DetailQuantityModal({
  ...
  const dataQuantity = useQuantityVat({
    params: {
      vat_invoice_id: vatInvoice?.vat_invoice_id,
      serial_number: vatInvoice?.serial_number,
      sort_order,
    },
    enabled: isOpen && !!vatInvoice?.vat_invoice_id,
  });
...
export function ListQuantityTable({
  ...
  const columns: TableColumnsType<IQuantityVat> = [
    {
    ...
    },
    {
      ...
      align: "center",
      render: () => (
        <Typography.Text>{vatInvoiceDetail.hs_code}</Typography.Text>
      ),
    },
    {
      ...
      render: () => (
        <Typography.Text>{vatInvoiceDetail.material_code}</Typography.Text>
      ),
    },
    {
      ...
      render: () => (
        <Typography.Paragraph
          ellipsis
          style={{ margin: 0 }}
          title={vatInvoiceDetail.material_name}
        >
          {vatInvoiceDetail.material_name}
        </Typography.Paragraph>
      ),
    },
    {
